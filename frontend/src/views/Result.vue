  ...
  <div class="result-container">
    ...
    <div class="page-header">
      <a-button class="back-button" size="large" @click="goBack">
        {{ historyId ? '← 返回历史记录' : '← 返回首页' }}
      </a-button>
      <a-space size="middle">
        <a-button v-if="!editMode" @click="toggleEditMode" type="default">
          ✏️ 编辑行程
        </a-button>
        <a-button v-else @click="saveChanges" type="primary" :loading="savingChanges">
          💾 保存修改
        </a-button>
    ...
    <div v-if="loadingPlan" class="result-state">
      <a-spin size="large" />
      <p>正在加载旅行计划...</p>
    </div>
    ...
    >
      <template #extra>
        <a-button type="primary" @click="router.push('/history')">返回历史记录</a-button>
        <a-button @click="loadPlan">重新加载</a-button>
      </template>
    </a-result>
                  ...
                  <a-list-item>
                    <a-card :title="item.name" size="small" class="attraction-card">
                      ...
                      <div v-if="editMode">
                        ...
                        <p><strong>门票(元):</strong></p>
                        <a-input-number v-model:value="item.ticket_price" :min="0" size="small" style="width: 100%" />
                      </div>
                      ...
                      <div v-else>
                        ...
                        <p v-if="item.ticket_price"><strong>门票:</strong> ¥{{ item.ticket_price }}</p>
...
import { getTripHistoryDetail, updateTripHistory } from '../services/api'
...
const historyId = computed(() => {
  const value = route.params.historyId
...
const loadPlan = async () => {
  ...
  if (historyId.value) {
    ...
    try {
      const response = await getTripHistoryDetail(historyId.value)
...
const goBack = () => {
  router.push(historyId.value ? '/history' : '/')
}
...
const saveChanges = async () => {
  ...
  try {
    if (historyId.value) {
      await updateTripHistory(historyId.value, tripPlan.value)
    }
