...
class TripHistoryDetail(TripHistorySummary):
    """旅行历史详情"""
    plan: TripPlan = Field(..., description="旅行计划")
...
class TripHistoryListResponse(BaseModel):
    ...
    data: List[TripHistorySummary] = Field(
        default_factory=list,
        description="旅行历史摘要列表",
    )


class TripHistoryDetailResponse(BaseModel):
    """旅行历史详情响应"""
    success: bool = Field(default=True, description="是否成功")
    message: str = Field(default="", description="消息")
    data: TripHistoryDetail = Field(..., description="旅行历史详情")


class TripHistoryUpdateRequest(BaseModel):
    """旅行历史更新请求"""
    plan: TripPlan = Field(..., description="更新后的旅行计划")
