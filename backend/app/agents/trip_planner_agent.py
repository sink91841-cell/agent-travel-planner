...
2. 格式必须完全正确,包括方括号和冒号
3. 参数用逗号分隔
4. 只返回旅游景点、博物馆、公园、古迹、自然景区等可游览地点
5. 排除餐馆、小吃店、美食街、酒店、商场等消费场所
"""
...
5. 每天必须包含早中晚三餐
6. 提供实用的旅行建议
7. attractions只能包含旅游景点,餐馆、小吃店和美食街必须放入meals
8. **必须包含预算信息**:
   - 景点门票价格(ticket_price)
...
class MultiAgentTripPlanner:
    ...
    def _build_attraction_query(self, request: TripRequest) -> str:
        """构建景点搜索查询 - 直接包含工具调用"""
        attraction_preferences = {"历史文化", "自然风光", "艺术", "休闲"}
        keywords = next(
            (
                preference
                for preference in request.preferences
                if preference in attraction_preferences
            ),
            "热门旅游景点"
        )
        ...
        # 直接返回工具调用格式
        query = (
            f"请使用amap_maps_text_search工具搜索{request.city}的{keywords}。"
            "只返回旅游景点、博物馆、公园、古迹和自然景区，"
            "排除餐馆、小吃店、美食街、酒店和商场。\n"
...
5. 返回完整的JSON格式数据
6. 景点的经纬度坐标要真实准确
7. attractions只能包含旅游景点,餐馆、小吃店和美食街不得放入 attractions,应放入 meals
"""
